...
<div class="admin-card" style="max-width:700px;margin:0 auto">
    <div class="card-header-custom">
        <h2 class="card-title-custom font-serif">{{ $program->title }}</h2>
    </div>
    <div class="card-body-custom">
        <form method="POST" action="{{ route('admin.weddings.program.update', [$wedding, $program]) }}">
            @csrf @method('PUT')
            <div class="row g-3">
                <div class="col-12">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="date" class="form-control datepicker" value="{{ old('date', $program->date?->format('Y-m-d')) }}">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Heure</label>
                    <input type="text" name="time" class="form-control timepicker" value="{{ old('time', $program->time ? substr($program->time, 0, 5) : '') }}">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Lieu</label>
                    <input type="text" name="venue_name" class="form-control" value="{{ old('venue_name', $program->venue_name) }}">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Adresse</label>
                    <input type="text" name="address" class="form-control" value="{{ old('address', $program->address) }}">
                </div>
                <div class="col-12">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="icon" class="form-control" value="{{ old('icon', $program->icon) }}">
                </div>
                <div class="col-md-3">
                    <label class="form-label">Ordre</label>
                    <input type="number" name="sort_order" class="form-control" value="{{ old('sort_order', $program->sort_order) }}">
                </div>
                <div class="col-12">
                    <div class="form-check form-switch">
                        <input type="hidden" name="is_published" value="0">
                        <input class="form-check-input" type="checkbox" name="is_published" value="1"
                               id="is_pub" {{ $program->is_published ? 'checked' : '' }}>
